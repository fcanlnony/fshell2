...
short
exec_builtin_cmd (char **array, const short FLAG, head_alias_t head,
                  const char *username, const char *cd_history)
{
  switch (FLAG)
    {
        ...
        if (array[1] == NULL)
        ...
        else if (!strncmp (array[1], "~", strlen ("~") * sizeof (char)))
          {
            ...
            if (!strcmp (username, "root"))
              asprintf (&tmp_dir, "/root/%s",
                        array[1] + strlen ("~") * sizeof (char));
            else
              asprintf (&tmp_dir, "/home/%s/%s", username,
                        array[1] + strlen ("~") * sizeof (char));
        ...
        if (array[3] == NULL)
        ...
        else if (array[3] != NULL)
          {
            ...
            for (i = 3; array[i] != NULL; i++)
              asprintf (&tmp_upload_alias, "%s %s", tmp_upload_alias,
                        array[i]);
        ...
        while (array[n] != NULL)
          {
            if (n >= 2)
              printf ("\n");
            char *tmp_put = getenv (array[n]),
                 *tmp_put2 = getalias_command (head, array[n]);
            if (tmp_put != NULL)
              printf ("env: %s => %s\n", array[n], tmp_put);
            else
                ...
                if (line > 1)
                  {
                    ...
                    cmd_t cmd
                        = (cmd_t)malloc (sizeof (struct cmd_or_pipe_chain));
                    cmd_t copy = cmd;
                    copy->next = NULL;
                    while (fgets (content, getsize, fp) != NULL)
                      {
                        copy->next = (cmd_t)malloc (
                            sizeof (struct cmd_or_pipe_chain));
                        copy = copy->next;
                        copy->sentence = (char *)calloc (
                            count_for_strlcpy (content), sizeof (char));
                        strlcpy (copy->sentence, content,
                                 count_for_strlcpy (content));
                    ...
                    char *array_sentence[FSHELL_INIT_FILE_ARRAY_NUM]
                        = { NULL };
                    while (copy2->sentence != NULL)
                      {
                        ...
                        if (FLAG != NON_BUILTIN_CMD)
                          exec_builtin_cmd (array_sentence, FLAG, head, NULL,
                                            NULL);
                        else
                    ...
                    if (fgets (content, size, fp) != NULL)
                      {
                        ...
                        char *array_content[FSHELL_INIT_FILE_ARRAY_NUM]
                            = { NULL };
                        array_parse (content, array_content);
                        int FLAG = check_builtin_cmd (array_content[0]);
                        if (FLAG != NON_BUILTIN_CMD)
                          exec_builtin_cmd (array_content, FLAG, head, NULL,
                                            NULL);
                        else
