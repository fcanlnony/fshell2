...
static alias_t
init_alias (alias_t alias_chain, const char *alias_name,
            const char *alias_command)
{
  alias_chain = (alias_t)malloc (sizeof (struct alias));
  char *alias_name_upload
      = (char *)calloc (count_for_strlcpy (alias_name), sizeof (char));
  char *alias_command_upload
      = (char *)calloc (count_for_strlcpy (alias_command), sizeof (char));
  strlcpy (alias_name_upload, alias_name, count_for_strlcpy (alias_name));
  strlcpy (alias_command_upload, alias_command,
           count_for_strlcpy (alias_command));
...
head_alias_t
upload_alias_node (head_alias_t alias_head, const char *alias_name,
                   const char *alias_command)
{
  if (alias_head->next != NULL)
      ...
      while (1)
        {
          if (!strcmp (current->alias_name, alias_name))
            {
              char *alias_command_upload = (char *)calloc (
                  count_for_strlcpy (alias_command), sizeof (char));
              strlcpy (alias_command_upload, alias_command,
                       count_for_strlcpy (alias_command));
            ...
            {
              current->next
                  = init_alias (current->next, alias_name, alias_command);
              return alias_head;
            }
        }
    }
    ...
    {
      alias_head->next
          = init_alias (alias_head->next, alias_name, alias_command);
      return alias_head;
    }
}
