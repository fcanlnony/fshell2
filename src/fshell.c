...
main(int argc, char** argv)
{
  ...
  while (1) {
      ...
      if (check_pipe(input) == false) {
        ...
        if (check_num != NON_BUILTIN_CMD)
          ...
          if (alias_cmd == NULL)
            ...
            char* tmp_alias_cmd_copy =
              (char*)calloc(count_for_strlcpy(alias_cmd), sizeof(char));
            strlcpy(
              tmp_alias_cmd_copy, alias_cmd, count_for_strlcpy(alias_cmd));
        ...
        char* copy_inputA =
          (char*)calloc(count_for_strlcpy(pipe_chain->sentence), sizeof(char));
        strlcpy(copy_inputA,
                pipe_chain->sentence,
                count_for_strlcpy(pipe_chain->sentence));
      ...
      while (1) {
        if (current->sentence != NULL) {
          if (check_pipe(current->sentence) == false) {
            ...
            if (check_num != NON_BUILTIN_CMD)
              exec_builtin_cmd(
                array, check_num, alias, user->username, cd_history);
            else {
              char* alias_cmd = getalias_command(alias, array[0]);
              if (alias_cmd == NULL)
                execvp_without_pipe(array);
            ...
            char** arrayB = (char**)calloc(array_pipe_num, sizeof(char*));
            char** arrayA_alias = (char**)calloc(array_pipe_num, sizeof(char*));
            char** arrayA_alias_tmp =
              (char**)calloc(array_pipe_num, sizeof(char*));
            char** input_arrayA = (char**)calloc(array_pipe_num, sizeof(char*));
            char** arrayB_alias = (char**)calloc(array_pipe_num, sizeof(char*));
            char** arrayB_alias_tmp =
              (char**)calloc(array_pipe_num, sizeof(char*));
            char** input_arrayB = (char**)calloc(array_pipe_num, sizeof(char*));
            char *tmp_aliasA_cmd_copy = NULL, *tmp_aliasB_cmd_copy = NULL;
            char* copy_inputA = (char*)calloc(
              count_for_strlcpy(pipe_chain->sentence), sizeof(char));
            strlcpy(copy_inputA,
                    pipe_chain->sentence,
                    count_for_strlcpy(pipe_chain->sentence));
            array_parse(pipe_chain->sentence, arrayA);
            ...
            if (alias_cmd == NULL) {
              ...
              tmp_aliasA_cmd_copy =
                (char*)calloc(count_for_strlcpy(alias_cmd), sizeof(char));
              strlcpy(
                tmp_aliasA_cmd_copy, alias_cmd, count_for_strlcpy(alias_cmd));
              array_parse(tmp_aliasA_cmd_copy, arrayA_alias_tmp);
              unsigned short i, k;
              for (i = 0; arrayA_alias_tmp[i] != NULL; i++) {
                arrayA_alias[i] = arrayA_alias_tmp[i];
            ...
            if (alias_cmd == NULL) {
              ...
              tmp_aliasB_cmd_copy =
                (char*)calloc(count_for_strlcpy(alias_cmd), sizeof(char));
              strlcpy(
                tmp_aliasB_cmd_copy, alias_cmd, count_for_strlcpy(alias_cmd));
              array_parse(tmp_aliasB_cmd_copy, arrayB_alias_tmp);
              unsigned short i, k;
              for (i = 0; arrayB_alias_tmp[i] != NULL; i++) {
                arrayB_alias[i] = arrayB_alias_tmp[i];
    ...
    cd_history =
      (char*)realloc(cd_history, strlen(user->userdir) * sizeof(char));
